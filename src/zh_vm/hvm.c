      ...
      //{
         ...
         while( ui-- )
         {
            ...
            if( scope == ZH_FS_STATIC_INIT )
            {
               ...
               // ovim se puni dinamicka tabela sa init static funkcijama
               // ne znam zasto ali s_pSymbols tabela izmjeni pFunPtr stavke kod ovih funkcija
               // u narednim vmInit sesijama
               // kako god, sada se u tabeli s_pDynItems nalaze ispravni func pointeri na STATIC_INIT funkcije
               PZH_DYNSYMBOL pDynSym = zh_dynsymGetCase( (pLastSymbols->pModuleSymbols + ui)->szName );
                     
               if (pDynSym && (s_vmInitPozivCount == 1)) {
                  // kod prvog poziva vmInit, s_pSymbols sadrzi ispravne pFunPtr-e
                  // njima punim dinamicku symbol tabelu
                  pDynSym->pSymbol->value.pFunPtr = (pLastSymbols->pModuleSymbols + ui)->value.pFunPtr;
               }
