...
PROCEDURE Main( cHost, cDatabase, cUser, cPass )
   ...
   LOCAL oServer := TPQServer():New( cHost, zh_defaultValue( cDatabase, "postgres" ), cUser, cPass )
   ...
   FOR i := 1 TO 10
      cQuery := "INSERT INTO test(code, dept, name, sales, tax, salary, budget, Discount, Creation, Description) " + ;
         "VALUES( " + zh_ntos( i ) + ", 2, 'TEST', 'y', 5, 3000, 1500.2, 7.5, '2003-12-17', 'Short Description about what ?')"
