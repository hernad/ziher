...
PROCEDURE Main( cHost, cDatabase, cUser, cPass )
   ...
   IF SQLConnect( cHost, zh_defaultValue( cDatabase, "postgres" ), cUser, cPass )
...
FUNCTION SQLOpen( cAlias, cQuery, xFetch, cOrder )
   ...
   IF Select( cAlias ) == 0
      zh_dbCreateTemp( cAlias, oQuery:Struct() )
   ELSE
      dbSelectArea( cAlias )
      zh_dbZap()
   ENDIF
...
FUNCTION SQLPrepare( cQuery, ... )
   ...
   IF PCount() >= 2
      ...
      FOR i := 1 TO PCount() - 1
         IF ( x := At( ":" + zh_ntos( i ), cQuery ) ) > 0
            cQuery := Stuff( cQuery, x, 0, "{" )
            cQuery := Stuff( cQuery, x + Len( zh_ntos( i ) ) + 2, 0, "}" )
         ENDIF
      NEXT
      ...
      FOR i := 2 TO PCount()
         ...
         cQuery := StrTran( cQuery, "{:" + zh_ntos( i - 1 ) + "}", x )
      NEXT
   ENDIF

   RETURN zh_StrReplace( cQuery, { ;
      "=="    => "="   , ;
      "!="    => "<>"  , ;
      ".and." => "and" , ;
      ".or."  => "or"  , ;
