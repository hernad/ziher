...
} ZH_FUNCINFO, * PZH_FUNCINFO;


/* NOTE: THIS TABLE MUST BE SORTED ALPHABETICALLY
 */
static const ZH_FUNCINFO s_stdFunc[] =
{
   { "AADD"      , 2,  2 },
   { "ABS"       , 1,  1 },
   { "ASC"       , 1,  1 },
...
ZH_BOOL zh_compFunCallCheck( ZH_COMP_DECL, const char * szFuncCall, int iArgs )
{
   ...
   if( iCmp == 0 )
   {
      const ZH_FUNCINFO * pFunc = &s_stdFunc[ uiFirst ];
          ...
          ( pFunc->iMaxParam != -1 && iArgs > pFunc->iMaxParam ) )
      {
         ...
         zh_compGenError( ZH_COMP_PARAM, zh_comp_szErrors, 'E', ZH_COMP_ERR_CHECKING_ARGS, szFuncCall, szMsg );
