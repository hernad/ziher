...
/* TODO:   "\n          -t<path>         path for temp file creation", */
   ...
   for( iLine = 0; iLine < ( int ) ZH_SIZEOFARRAY( s_szOptions ); iLine++ )
      zh_compOutStd( ZH_COMP_PARAM, s_szOptions[ iLine ] );
}

/* List of compatibility/features modes */
void zh_compPrintModes( ZH_COMP_DECL )
{
   static const char * s_szOptions[] =
   {
   ...
   {
      zh_compOutStd( ZH_COMP_PARAM, s_szOptions[ iLine ] );
      if( iLine < ( int ) ZH_SIZEOFARRAY( s_flags ) &&
          ( s_flags[ iLine ] < 0 ? ZH_COMP_ISSUPPORTED( ~s_flags[ iLine ] ) == 0 :
                                   ZH_COMP_ISSUPPORTED( s_flags[ iLine ] ) != 0 ) )
         zh_compOutStd( ZH_COMP_PARAM, " (default)" );
   }
}

void zh_compPrintCredits( ZH_COMP_DECL )
{
   zh_compOutStd( ZH_COMP_PARAM,
         "\n"
...
void zh_compPrintLogo( ZH_COMP_DECL )
{
   char * szVer = zh_verZiher();
