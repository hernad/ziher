...
ZH_FUNC( XTOC )
{
   ...
   if( pItem )
   {
      ...
      {
         char szDate[ 9 ];
         zh_retc( zh_itemGetDS( pItem, szDate ) );
      }
      else if( ZH_IS_TIMESTAMP( pItem ) )
      {
         char szDateTime[ 18 ];
         zh_retc( zh_itemGetTS( pItem, szDateTime ) );
      }
      else if( ZH_IS_NUMERIC( pItem ) )
      {
         ...
         ZH_PUT_LE_DOUBLE( buf, d );
         zh_retclen( buf, sizeof( buf ) );
      }
      else if( ZH_IS_LOGICAL( pItem ) )
         zh_retclen( zh_itemGetL( pItem ) ? "T" : "F", 1 );
      else
         zh_itemReturn( pItem );
   }
}
