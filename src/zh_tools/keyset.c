...
static void SetGet( int iFlag )
{
   ...
   if( gtInfo.pResult )
   {
      iState = zh_itemGetNI( gtInfo.pResult );
   ...
   if( gtInfo.pNewVal )
      zh_itemRelease( gtInfo.pNewVal );
   if( gtInfo.pResult )
      zh_itemRelease( gtInfo.pResult );

   zh_retl( ( iState & iFlag ) != 0 );
}

ZH_FUNC( KSETINS )
{
   SetGet( ZH_GTI_KBD_INSERT );
}

ZH_FUNC( KSETCAPS )
{
   SetGet( ZH_GTI_KBD_CAPSLOCK );
}

ZH_FUNC( KSETNUM )
{
   SetGet( ZH_GTI_KBD_NUMLOCK );
}

ZH_FUNC( KSETSCROLL )
{
   SetGet( ZH_GTI_KBD_SCROLOCK );
}
