...
ZH_FUNC( COMPLEMENT )
{
   ...
   if( pItem )
   {
      if( ZH_IS_STRING( pItem ) )
      {
         ...
         if( nLen > 0 )
         {
            const char * szSrc = zh_itemGetCPtr( pItem );
         ...
         zh_retdl( 4537847 - zh_itemGetDL( pItem ) );
      else if( ZH_IS_TIMESTAMP( pItem ) )
         zh_rettd( 4537847.0 - zh_itemGetTD( pItem ) );
      else if( ZH_IS_NUMINT( pItem ) )
         zh_retnint( -zh_itemGetNInt( pItem ) );
      else if( ZH_IS_NUMERIC( pItem ) )
      {
         int iWidth, iDec;
         double dValue;

         dValue = zh_itemGetND( pItem );
         zh_itemGetNLen( pItem, &iWidth, &iDec );
         zh_retndlen( -dValue, iWidth, iDec );
      }
