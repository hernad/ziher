...
ZH_FUNC( STRDIFF )
{
   ...
   if( ZH_ISCHAR( 1 ) || ZH_ISCHAR( 2 ) )
   {
      ...
      int *        piPenalty;
      ZH_SIZE      sRowCnt, sColCnt;

      if( ZH_ISCHAR( 1 ) )
      {
         pcStr1   = zh_parc( 1 );
         sStrLen1 = zh_parclen( 1 );
      }
      else
      {
         pcStr1   = "";
          ...
          ( ( double ) sizeof( int ) ) >= ( double ) UINT_MAX )
      {
         ...
         if( iArgErrorMode != CT_ARGERR_IGNORE )
            ct_error( ( ZH_USHORT ) iArgErrorMode, EG_ARG, CT_ERROR_STRDIFF, NULL,
      ...
      /* get penalty points */
      iReplace = zh_parnidef( 3, 3 );
      iDelete = zh_parnidef( 4, 6 );
      iInsert = zh_parnidef( 5, 1 );

      piPenalty = ( int * ) zh_xgrab( ( sStrLen1 + 1 ) *
      ...
      zh_retni( MATRIXELEMENT( sStrLen1, sStrLen2 ) );
      zh_xfree( piPenalty );
   }
   else
   {
      ...
      if( iArgErrorMode != CT_ARGERR_IGNORE )
         pSubst = ct_error_subst( ( ZH_USHORT ) iArgErrorMode, EG_ARG,
                                  CT_ERROR_STRDIFF, NULL, ZH_ERR_FUNCNAME, 0,
