/*
 * Example of Ziher codepage using UTF-16 little endian encoding
 *
...
static ZH_CDP_GET_FUNC( UTF16LE_get )
{
   ...
   if( *pnIndex + 1 < nLen )
   {
      *wc = ZH_GET_LE_UINT16( &pSrc[ *pnIndex ] );
      *pnIndex += 2;
      return ZH_TRUE;
   }
   else
...
static ZH_CDP_PUT_FUNC( UTF16LE_put )
{
   ...
   if( *pnIndex + 1 < nLen )
   {
      ZH_PUT_LE_UINT16( &pDst[ *pnIndex ], wc );
      *pnIndex += 2;
      return ZH_TRUE;
   }
   return ZH_FALSE;
...
#define ZH_CP_ID              UTF16LE
#define ZH_CP_INFO            "UTF-16 little endian"
#define ZH_CP_UNITB           ZH_UNITB_437

#define ZH_CP_GET_FUNC        UTF16LE_get
#define ZH_CP_PUT_FUNC        UTF16LE_put
#define ZH_CP_LEN_FUNC        UTF16LE_len
