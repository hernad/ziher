...
/* mingw-w64 6.1.0 x64-hosted multilib will crash zh_pp executable
...
#if defined( ZH_GCC_HAS_OPTIMIZE ) && ( ZH_GCC_VER == 601 ) \
    && defined( __MINGW32__ ) && defined( ZH_CPU_X86_64 )
...
#if defined( ZH_LONG_LONG_OFF ) && ! defined( __NO_LONGLONG__ )
#  define __NO_LONGLONG__
#endif


#if ! defined( ZH_USE_CRTL_SNPRINTF )
...
#  define _x_longlong         ZH_LONGLONG
#  define _x_ulonglong        ZH_ULONGLONG
#else
   ...
   if( plst->repeat )
   {
      ...
      if( iArg > plst->size )
      {
         ...
         if( prev_size == _ARGBUF_SIZE )
            plst->arglst = ( v_param * ) memcpy( zh_xgrab( plst->size * sizeof( v_param ) ),
                                                 plst->arglst, _ARGBUF_SIZE * sizeof( v_param ) );
         else
            plst->arglst = ( v_param * ) zh_xrealloc( plst->arglst, plst->size * sizeof( v_param ) );
...
#ifdef _ZH_WRAP_MODFL_
_x_long_dbl _zh_modfl( _x_long_dbl x, _x_long_dbl * p )
{
   _x_double pd;
...
static int _zh_strnlen( const char * str, int len )
{
   int i = 0;
                       ...
                       const _x_str str, int flags, int width, int precision )
{
   ...
   if( precision < 0 )
      ...
      precision = ( int ) _zh_strnlen( str, precision );
...
int zh_printf_params( const char * format )
{
   int iParam = 0, iMax = 0;
   char c;
...
int zh_vsnprintf( char * buffer, size_t bufsize, const char * format, va_list ap )
{
   va_list args;
   size_t size;
   char c;
               ...
               switch( c )
               {
                  ...
                  case 'F':   /* double decimal notation */
                     if( length == _L_LONGDOUBLE_ )
                     {
                        ...
                        ZH_NUMTYPEL( value, argval.value.as_x_long_dbl );
                     }
                     else
                     {
                        ...
                        argval.value.as_x_long_dbl = ( _x_long_dbl )
                           ( ( value & ( _ZH_NUM_NAN | _ZH_NUM_PINF | _ZH_NUM_NINF ) ) == 0 ? d : 0 );
                     }
                     if( value & _ZH_NUM_NAN )
                        size = put_str( buffer, bufsize, size,
                                        c == 'f' ?
                                        ( ( flags & _F_SIGN ) ? "+nan": "nan" ) :
                                        ( ( flags & _F_SIGN ) ? "+NAN": "NAN" ) ,
                                        flags, width, -1 );
                     else if( value & _ZH_NUM_PINF )
                        size = put_str( buffer, bufsize, size,
                                        c == 'f' ?
                                        ( ( flags & _F_SIGN ) ? "+inf": "inf" ) :
                                        ( ( flags & _F_SIGN ) ? "+INF": "INF" ),
                                        flags, width, -1 );
                     else if( value & _ZH_NUM_NINF )
                        size = put_str( buffer, bufsize, size,
                                        c == 'f' ? "-inf" : "-INF",
                                        flags, width, -1 );
                     else
                  ...
                  case 'p':   /* void * pointer */
                     ...
                     if( argval.value.as_x_ptr )
                        size = put_hex( buffer, bufsize, size, ( ZH_PTRUINT ) argval.value.as_x_ptr,
...
#else /* defined( ZH_USE_CRTL_SNPRINTF ) */

#undef _ZH_SNPRINTF_ADD_EOS

/* NOTE: The full size of the buffer is expected as nSize. [vszakats] */
int zh_vsnprintf( char * buffer, size_t nSize, const char * format, va_list arglist )
{
   int result;
...
#endif /* ZH_USE_CRTL_SNPRINTF */

int zh_snprintf( char * buffer, size_t bufsize, const char * format, ... )
{
   va_list ap;
   int iResult;

   va_start( ap, format );
   iResult = zh_vsnprintf( buffer, bufsize, format, ap );
   va_end( ap );
