...
STATIC FUNCTION DefError( oError )
   ...
   IF ! Empty( oError:osCode )
      cOSError := zh_StrFormat( "(OS Error %1$d)", oError:osCode )
   ENDIF
   ...
   DO WHILE ! Empty( ProcName( ++n ) )
      OutErr( zh_eol() )
      OutErr( zh_StrFormat( "Called from %1$s(%2$d)  ", ;
         ProcName( n ), ;
         ProcLine( n ) ) )
   ENDDO
...
STATIC FUNCTION ErrorMessage( oError )
   ...
   IF ZH_ISSTRING( oError:subsystem )
   ...
   IF ZH_ISNUMERIC( oError:subCode )
      cMessage += "/" + zh_ntos( oError:subCode )
   ELSE
      cMessage += "/???"
   ENDIF

   // add error description if available
   IF ZH_ISSTRING( oError:description )
