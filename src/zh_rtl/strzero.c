...
ZH_FUNC( STRZERO )
{
   ...
   if( iParams >= 1 && iParams <= 3 )
   {
      ...
      PZH_ITEM pWidth  = NULL;
      PZH_ITEM pDec    = NULL;

      if( iParams >= 2 )
      {
         pWidth = zh_param( 2, ZH_IT_NUMERIC );
         if( pWidth == NULL )
            pNumber = NULL;
         else if( iParams >= 3 )
         {
            pDec = zh_param( 3, ZH_IT_NUMERIC );
            if( pDec == NULL )
               pNumber = NULL;
         }
      }
      ...
      if( pNumber )
      {
         char * szResult = zh_itemStr( pNumber, pWidth, pDec );
         ...
         zh_errRT_BASE_SubstR( EG_ARG, 6003, NULL, ZH_ERR_FUNCNAME, ZH_ERR_ARGS_BASEPARAMS );
   }
}
