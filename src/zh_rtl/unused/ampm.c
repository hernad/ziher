...
ZH_FUNC( AMPM )
{
   ZH_SIZE nTimeLen  = zh_parclen( 1 );
   char *  pszResult = ( char * ) zh_xgrab( ZH_MAX( nTimeLen, 2 ) + 3 + 1 );
   int     iHour     = 0;
   ZH_BOOL bAM;

   if( nTimeLen )
   {
      const char * pszTime = zh_parc( 1 );
      memcpy( pszResult, pszTime, nTimeLen );
      iHour = ( int ) zh_strVal( pszTime, nTimeLen );
   }
   ...
   zh_retclen_buffer( pszResult, nTimeLen + 3 );
}
