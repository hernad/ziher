...
static ZH_CODEPAGE_GET_FUNC( GBK_get )
{
   *wc = 0;
   if( *pnIndex < nLen )
   {
      ...
      if( uc >= ( ZH_GBK_FIRST >> 8 ) && uc <= ( ZH_GBK_LAST >> 8 ) &&
          *pnIndex < nLen )
      {
         *wc = s_gbk_to_ucs16( ( ( int ) uc << 8 ) | ( ZH_UCHAR ) pSrc[ *pnIndex ] );
         if( *wc )
         {
            ( *pnIndex )++;
            return ZH_TRUE;
...
static ZH_CODEPAGE_PUT_FUNC( GBK_put )
{
   if( *pnIndex < nLen )
   {
      ...
      if( gb18030 )
      {
         if( *pnIndex + 1 < nLen )
         {
            ZH_PUT_BE_UINT16( &pDst[ ( *pnIndex ) ], gb18030 );
            *pnIndex += 2;
            return ZH_TRUE;
         }
      }
...
#define ZH_CP_ID              GBK
#define ZH_CP_INFO            "CP936"
#define ZH_CP_UNITB           ZH_UNITB_437

#define ZH_CP_GET_FUNC        GBK_get
#define ZH_CP_PUT_FUNC        GBK_put
#define ZH_CP_LEN_FUNC        GBK_len
