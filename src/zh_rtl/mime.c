...
{
   ZH_SIZE            pos;       /* Position in stream from which the match begins */
...
typedef struct tag_mime_ext
{
   ...
   ZH_USHORT    flags;      /* flags for confrontation */
   const char * mime_type;  /* MIME type if complete */
} EXT_MIME_ENTRY;
...
{
   unsigned int uiFirst = 0, uiLast = ZH_SIZEOFARRAY( s_extMimeTable );
   char szExt[ 16 ];

   if( *szFileExt == '.' )
      ++szFileExt;
   zh_strncpyLower( szExt, szFileExt, sizeof( szExt ) - 1 );
...
static const char * s_findMimeStringInTree( const char * cData, ZH_SIZE nLen, int iElem )
{
   const MIME_ENTRY * elem = s_mimeTable + iElem;
   ZH_SIZE nPos     = elem->pos;
   ZH_SIZE nDataLen = strlen( elem->pattern );
   ...
   if( nPos < nLen && ( nLen - nPos ) >= nDataLen )
   {
      ...
      {
         if( ( *elem->pattern == 0 && cData[ nPos ] == 0 ) || zh_strnicmp( cData + nPos, elem->pattern, nDataLen ) == 0 )
         {
...
static const char * s_findStringMimeType( const char * cData, ZH_SIZE nLen )
{
   unsigned int uiCount;

   for( uiCount = 0; uiCount < ZH_SIZEOFARRAY( s_mimeTable ); uiCount++ )
   {
      const MIME_ENTRY * elem = s_mimeTable + uiCount;
      ZH_SIZE nPos     = elem->pos;
      ZH_SIZE nDataLen = ( ZH_SIZE ) strlen( elem->pattern );
...
static const char * s_findFileMimeType( PZH_FILE fileIn )
{
   char buf[ 512 ];

   ZH_FOFFSET nPos = zh_fileSeek( fileIn, 0, FS_RELATIVE );
   ZH_SIZE    nLen = zh_fileResult( zh_fileReadAt( fileIn, buf, sizeof( buf ), 0 ) );

   if( nLen > 0 )
   {
      zh_fileSeek( fileIn, nPos, FS_SET );
...
/* zh_mimeStr( <cContent>, [<cDefaultType>] ) --> cMimeType */
ZH_FUNC( ZH_MIMESTR )
{
   ...
   if( pData )
   {
      const char * magic_type = s_findStringMimeType( zh_itemGetCPtr( pData ), zh_itemGetCLen( pData ) );

      if( magic_type )
         zh_retc_const( magic_type );
      else if( ZH_ISCHAR( 2 ) )
         zh_retc( zh_parc( 2 ) );
      else
         zh_retc_const( "application/unknown" );
   }
   else
      zh_errRT_BASE_SubstR( EG_ARG, 3012, NULL, ZH_ERR_FUNCNAME, ZH_ERR_ARGS_BASEPARAMS );
}

/* zh_mimeFName( <cFileName>, [<cDefaultType>] ) --> cMimeType */
ZH_FUNC( ZH_MIMEFNAME )
{
   const char * fname = zh_parc( 1 );

   if( fname )
   {
      PZH_FNAME pFileName = zh_fsFNameSplit( fname );
      const char * ext_type = pFileName->szExtension ? s_findExtMimeType( pFileName->szExtension ) : NULL;
      zh_xfree( pFileName );

       if( ext_type )
         zh_retc_const( ext_type );
      else if( ZH_ISCHAR( 2 ) )
         zh_retc( zh_parc( 2 ) );
      else
         zh_retc_const( "application/unknown" );
...
/* zh_mimeFile( <cFileName|hFile|nFile>, [<cDefaultType>] ) --> cMimeType */
ZH_FUNC( ZH_MIMEFILE )
{
   PZH_ITEM pFile = zh_param( 1, ZH_IT_STRING | ZH_IT_POINTER | ZH_IT_NUMERIC );
   ...
   if( pFile )
   {
      ...
      if( ZH_IS_STRING( pFile ) )
      {
         const char * fname = zh_itemGetCPtr( pFile );
         ...
         if( ( fileIn = zh_fileExtOpen( fname, NULL,
      ...
      else if( zh_fileItemGet( pFile ) )
         magic_type = s_findFileMimeType( zh_fileItemGet( pFile ) );
      else
      {
         PZH_FILE fileIn = zh_fileFromHandle( zh_numToHandle( zh_itemGetNInt( pFile ) ) );
         magic_type = s_findFileMimeType( fileIn );
         zh_fileDetach( fileIn );
      }
