...
METHOD New( cText, nTop, nLeft, nBottom, nRight, lEditMode, nLineLength, ;
   ...
   ::lEditAllow := zh_defaultValue( lEditMode, .T. )

   IF ZH_ISNUMERIC( nLineLength ) .AND. nLineLength >= 1
      ::lWordWrap := .T.
      ::nWordWrapCol := nLineLength
   ENDIF

   IF ZH_ISNUMERIC( nTabSize ) .AND. nTabSize >= 1
      ::nTabWidth := Max( nTabSize, 2 )
   ENDIF

   ::LoadText( zh_defaultValue( cText, "" ) )
   ::InsertState( Set( _SET_INSERT ) )

   ::nRow := zh_defaultValue( nTextRow, 1 )
   ::nCol := zh_defaultValue( nTextCol, 0 ) + 1
   ::nFirstRow := ::nRow - zh_defaultValue( nWndRow, 0 )
   ::nFirstCol := ::nCol - zh_defaultValue( nWndCol, 0 )
   ::Resize( zh_defaultValue( nTop, 0 ), ;
             zh_defaultValue( nLeft, 0 ), ;
             zh_defaultValue( nBottom, MaxRow() ), ;
             zh_defaultValue( nRight, MaxCol() ) )
...
METHOD Resize( nTop, nLeft, nBottom, nRight ) CLASS HBEditor
   ...
   IF ZH_ISNUMERIC( nTop )
      ::nTop := nTop
   ENDIF
   IF ZH_ISNUMERIC( nLeft )
      ::nLeft := nLeft
   ENDIF
   IF ZH_ISNUMERIC( nBottom )
      ::nBottom := nBottom
   ENDIF
   IF ZH_ISNUMERIC( nRight )
      ::nRight := nRight
   ENDIF
...
METHOD LoadFile( cFileName ) CLASS HBEditor
   RETURN ::LoadText( zh_MemoRead( ::cFile := cFileName ) )
...
METHOD SaveFile() CLASS HBEditor
   RETURN ! ::cFile == "" .AND. ;
          ! ::lDirty := ! zh_MemoWrit( ::cFile, ::GetText() )
...
METHOD InsertLine( cLine, lSoftCR, nRow ) CLASS HBEditor
   ...
   zh_AIns( ::aText, nRow, HBTextLine():New( cLine, lSoftCR ), .T. )
...
METHOD GetText( lSoftCR ) CLASS HBEditor
   ...
   cEOL := zh_eol()
   cSoftCR := iif( ::lWordWrap, iif( zh_defaultValue( lSoftCR, .F. ), ;
                                     Chr( 141 ) + Chr( 10 ), "" ), cEOL )
   cText := ""
...
METHOD Display() CLASS HBEditor
   ...
   nCount := ::nNumRows
   DO WHILE --nCount >= 0
      zh_DispOutAt( nRow++, ::nLeft, SubStrPad( ::GetLine( nLine ), ::nFirstCol, ::nNumCols ), ::LineColor( nLine++ ) )
   ENDDO
   DispEnd()
...
METHOD RefreshLine() CLASS HBEditor
   ...
   zh_DispOutAt( ::Row(), ::nLeft, SubStrPad( ::GetLine( ::nRow ), ::nFirstCol, ::nNumCols ), ::LineColor( ::nRow ) )
...
METHOD GoTo( nRow, nCol, nRefreshMode )
   ...
   zh_default( @nRefreshMode, _REFRESH_NONE )
...
METHOD MoveCursor( nKey ) CLASS HBEditor
   ...
   CASE K_CTRL_RIGHT
      ...
      IF nKey != K_CTRL_RIGHT .AND. zh_keyVal( nKey ) != ZH_KX_RIGHT
         RETURN .F.
      ENDIF
   ...
   CASE K_CTRL_END
      ...
      IF nKey != K_CTRL_END .AND. zh_keyVal( nKey ) != ZH_KX_END
         RETURN .F.
      ENDIF
...
METHOD Edit( nPassedKey ) CLASS HBEditor
   ...
   DO WHILE ! ::lExitEdit
      ...
      CASE ! ( cKey := iif( nKeyStd == K_TAB .AND. Set( _SET_INSERT ), ;
         ...
         ::lDirty := .T.
         oLine := ::aText[ ::nRow ]
         IF ( nPos := ::nCol - zh_ULen( oLine:cText ) - 1 ) > 0
            oLine:cText += Space( nPos )
         ENDIF
         oLine:cText := zh_UStuff( oLine:cText, ::nCol, ;
                               iif( Set( _SET_INSERT ), 0, 1 ), cKey )
         ::nCol += zh_ULen( cKey )
         IF ::lWordWrap .AND. zh_ULen( oLine:cText ) > ::nWordWrapCol
            ::ReformParagraph()
         ELSE
      ...
      CASE nKeyStd == K_ENTER
         IF Set( _SET_INSERT )
            ::lDirty := .T.
            oLine := ::aText[ ::nRow ]
            ::InsertLine( zh_USubStr( oLine:cText, ::nCol ), oLine:lSoftCR, ::nRow + 1 )
            oLine:cText := zh_ULeft( oLine:cText, ::nCol - 1 )
            oLine:lSoftCR := .F.
      ...
      CASE nKeyStd == K_BS
         IF ::nCol > 1
            ::lDirty := .T.
            ::aText[ ::nRow ]:cText := zh_UStuff( ::aText[ ::nRow ]:cText, --::nCol, 1, "" )
            ::GoTo( ::nRow, ::nCol, _REFRESH_LINE )
         ENDIF
      ...
      CASE nKeyStd == K_DEL
         IF ::nRow < ::LineCount .OR. ::nCol <= ::LineLen( ::nRow )
            ::lDirty := .T.
            oLine := ::aText[ ::nRow ]
            IF ::nCol <= zh_ULen( oLine:cText )
               oLine:cText := zh_UStuff( oLine:cText, ::nCol, 1, "" )
               ::GoTo( ::nRow, ::nCol, _REFRESH_LINE )
            ELSE
               IF ::nCol > zh_ULen( oLine:cText ) + 1
                  oLine:cText += Space( ::nCol - zh_ULen( oLine:cText ) - 1 )
               ENDIF
      ...
      CASE nKeyStd == K_CTRL_T
         IF ( nPos := SkipWord( ::GetLine( ::nRow ), ::nCol ) - ::nCol ) > 0
            ::lDirty := .T.
            ::aText[ ::nRow ]:cText := zh_UStuff( ::aText[ ::nRow ]:cText, ::nCol, nPos, "" )
            ::GoTo( ::nRow, ::nCol, _REFRESH_LINE )
         ENDIF
      ...
      CASE ( zh_bitAnd( zh_keyMod( nKey ), ZH_KF_CTRL ) != 0 .AND. Upper( zh_keyChar( zh_keyVal( nKey ) ) ) == "B" ) .OR. ;
           nKeyStd == K_ALT_B  /* K_ALT_B is not Cl*pper compatible */
         ::ReformParagraph()

      CASE ( zh_bitAnd( zh_keyMod( nKey ), ZH_KF_CTRL ) != 0 .AND. Upper( zh_keyChar( zh_keyVal( nKey ) ) ) == "W" ) .OR. ;
...
METHOD ReformParagraph() CLASS HBEditor
   ...
   nCol := Min( zh_ULen( ::aText[ ::nRow ]:cText ) + 1, ::nCol )
   ...
   nLine := ::nRow
   zh_MLEval( cText, {| cLine, lSoftCR | ::InsertLine( cLine, lSoftCR, nLine++ ) }, ;
...
METHOD Hilite() CLASS HBEditor
   ...
   LOCAL cEnhanced := ;
      zh_tokenGet( ::cColorSpec, 2, "," ) + "," + ;
      zh_tokenGet( ::cColorSpec, 1, "," )

   ::SetColor( cEnhanced + zh_BRight( ::cColorSpec, zh_BLen( ::cColorSpec ) - zh_BLen( cEnhanced ) ) )
...
METHOD DeHilite() CLASS HBEditor
   ...
   ::SetColor( cStandard + zh_BRight( ::cColorSpec, zh_BLen( ::cColorSpec ) - zh_BLen( cStandard ) ) )
...
STATIC FUNCTION Text2Array( cText, nWordWrapCol, nTabWidth )
   ...
   zh_MLEval( cText, {| cLine, lSoftCR | AAdd( aArray, HBTextLine():New( cLine, lSoftCR ) ) }, ;
...
STATIC FUNCTION SubStrPad( cText, nFrom, nLen )
   RETURN zh_UPadR( zh_USubStr( cText, nFrom, nLen ), nLen )
...
STATIC FUNCTION SkipWord( cText, nPos )
   ...
   DO WHILE nPos < zh_ULen( cText ) .AND. zh_USubStr( cText, nPos, 1 ) == " "
      ++nPos
   ENDDO
   IF ( nPos := zh_UAt( " ", cText, nPos ) ) == 0
      nPos := zh_ULen( cText ) + 1
   ENDIF
...
STATIC FUNCTION NextWord( cText, nPos )
   ...
   IF ( nPos := zh_UAt( " ", cText, nPos ) ) == 0
      nPos := zh_ULen( cText ) + 1
   ELSE
      DO WHILE zh_USubStr( cText, ++nPos, 1 ) == " "
      ENDDO
   ENDIF
...
STATIC FUNCTION PrevWord( cText, nPos )
   ...
   DO WHILE nPos > 1 .AND. zh_USubStr( cText, --nPos, 1 ) == " "
   ENDDO
   DO WHILE nPos > 1 .AND. ! zh_USubStr( cText, nPos - 1, 1 ) == " "
      --nPos
   ENDDO
