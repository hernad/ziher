...
/* NOTE: CA-Cl*pper 5.x uses DevPos(), DevOut() to display messages
         on screen. Ziher uses Disp*() functions only. [vszakats] */
...
METHOD PROCEDURE New( cLBLName, lPrinter, cAltFile, lNoConsole, bFor, ;
   ...
   ELSE
      /* NOTE: CA-Cl*pper does an RTrim() on the filename here,
               but in Ziher we're using _SET_TRIMFILENAME. [vszakats] */
      IF Set( _SET_DEFEXTENSIONS )
         cLBLName := zh_FNameExtSetDef( cLBLName, ".lbl" )
      ENDIF
   ENDIF
...
METHOD ExecuteLabel() CLASS HBLabelForm
   ...
   FOR EACH aField IN ::aLabelData[ LBL_FIELDS ]
      ...
      IF aField != NIL
         ...
         cBuffer := ;
            zh_UPadR( Eval( aField[ LF_EXP ] ), ::aLabelData[ LBL_WIDTH ] ) + ;
...
METHOD LoadLabel( cLblFile ) CLASS HBLabelForm
   ...
   // Open the label file
   hFile := zh_vfOpen( cLblFile, ZH_FO_DEFAULTS )
   ...
   IF hFile == NIL
      ...
      Eval( ErrorBlock(), err )
   ELSE
      IF zh_vfRead( hFile, @cBuff, BUFFSIZE ) > 0 .AND. FError() == 0
         // Load label dimension into aLabel
         aLabel[ LBL_REMARK  ] := zh_BSubStr( cBuff, REMARKOFFSET, REMARKSIZE )
         aLabel[ LBL_HEIGHT  ] := Bin2W( zh_BSubStr( cBuff, HEIGHTOFFSET, HEIGHTSIZE ) )
         aLabel[ LBL_WIDTH   ] := Bin2W( zh_BSubStr( cBuff, WIDTHOFFSET, WIDTHSIZE ) )
         aLabel[ LBL_LMARGIN ] := Bin2W( zh_BSubStr( cBuff, LMARGINOFFSET, LMARGINSIZE ) )
         aLabel[ LBL_LINES   ] := Bin2W( zh_BSubStr( cBuff, LINESOFFSET, LINESSIZE ) )
         aLabel[ LBL_SPACES  ] := Bin2W( zh_BSubStr( cBuff, SPACESOFFSET, SPACESSIZE ) )
         aLabel[ LBL_ACROSS  ] := Bin2W( zh_BSubStr( cBuff, ACROSSOFFSET, ACROSSSIZE ) )

         FOR i := 1 TO aLabel[ LBL_HEIGHT ]

            // Get the text of the expression
            cFieldText := RTrim( zh_BSubStr( cBuff, nOffset, FIELDSIZE ) )
            nOffset += FIELDSIZE
            ...
            ELSE
               AAdd( aLabel[ LBL_FIELDS ], { ;
                  /* LF_EXP */ zh_macroBlock( cFieldText ), ;
                  /* LF_TEXT */ cFieldText, ;
                  /* LF_BLANK */ .T. } )
            ENDIF
         NEXT
