...
FUNCTION AChoice( nTop, nLeft, nBottom, nRight, acItems, xSelect, xUserFunc, nPos, nHiLiteRow )
   ...
   nTop := Int( zh_defaultValue( nTop, 0 ) )
   nBottom := Int( zh_defaultValue( nBottom, 0 ) )
   ...
   IF ! ZH_ISARRAY( acItems ) .OR. Len( acItems ) == 0
   ...
   IF ! ZH_ISARRAY( xSelect ) .AND. ! ZH_ISLOGICAL( xSelect )
      xSelect := .T.               // Array or logical, what is selectable
   ENDIF

   zh_default( @nPos, 1 )          // The number of the selected item
   zh_default( @nHiLiteRow, 0 )    // The row to be highlighted

   nNumCols := nRight - nLeft + 1
   nNumRows := nBottom - nTop + 1

   IF ZH_ISARRAY( xSelect )
      alSelect := xSelect
   ELSE
   ...
   DO WHILE ! lFinished
      ...
      CASE ( ! lUserFunc .OR. nMode == AC_EXCEPT ) .AND. ;
           ! ( cKey := Upper( zh_keyChar( nKeyStd ) ) ) == ""

         // Find next selectable item
         FOR nNewPos := nPos + 1 TO nItems
            IF Ach_Select( alSelect, nNewPos ) .AND. zh_LeftEqI( acItems[ nNewPos ], cKey )
               EXIT
            ENDIF
         NEXT
         IF nNewPos == nItems + 1
      ...
      IF lUserFunc
         ...
         IF ZH_ISNUMERIC( nUserFunc := Do( xUserFunc, nMode, nPos, nPos - nAtTop ) )
...
STATIC PROCEDURE DispPage( acItems, alSelect, nTop, nLeft, nRight, nNumRows, nPos, nAtTop, nArrLen, nRowsClr )
   ...
   FOR nCntr := 1 TO Min( nNumRows, nRowsClr )
      ...
      ELSE
         ColorSelect( CLR_STANDARD )
         zh_DispOutAt( nRow, nLeft, Space( nRight - nLeft + 1 ) )
      ENDIF
   NEXT
...
STATIC PROCEDURE DispLine( cLine, nRow, nCol, lSelect, lHiLite, nNumCols )
   ...
   ColorSelect( iif( lSelect .AND. ZH_ISSTRING( cLine ), ;
      iif( lHiLite, CLR_ENHANCED, CLR_STANDARD ), CLR_UNSELECTED ) )

   zh_DispOutAt( nRow, nCol, iif( ZH_ISSTRING( cLine ), zh_UPadR( cLine, nNumCols ), Space( nNumCols ) ) )
   IF lHiLite
      SetPos( nRow, nCol )
   ENDIF
...
STATIC FUNCTION Ach_Limits( /* @ */ nFrstItem, /* @ */ nLastItem, /* @ */ nItems, alSelect, acItems )
   ...
   FOR nCntr := 1 TO Len( acItems )
      IF ZH_ISSTRING( acItems[ nCntr ] ) .AND. ! acItems[ nCntr ] == ""
         nItems++
...
STATIC FUNCTION Ach_Select( alSelect, nPos )
   ...
   IF nPos >= 1 .AND. nPos <= Len( alSelect )
      sel := alSelect[ nPos ]
      DO CASE
      CASE ZH_ISEVALITEM( sel )
         sel := Eval( sel )
      CASE ZH_ISSTRING( sel ) .AND. ! Empty( sel )
         sel := Eval( zh_macroBlock( sel ) )
      ENDCASE
      IF ZH_ISLOGICAL( sel )
         RETURN sel
      ENDIF
