...
CREATE CLASS ListBox FUNCTION HBListBox
   ...
   METHOD New( nTop, nLeft, nBottom, nRight, lDropDown )   /* NOTE: This method is a Ziher extension [vszakats] */
...
METHOD display() CLASS ListBox
   ...
   IF ::lHasFocus
      cHotBox   := ::cHotBox
      cColor3   := zh_ColorIndex( ::cColorSpec, 2 )
      cColor4   := zh_ColorIndex( ::cColorSpec, 3 )
      cColorAny := iif( ::lIsOpen, zh_ColorIndex( ::cColorSpec, 1 ), zh_ColorIndex( ::cColorSpec, 3 ) )
   ELSE
      cHotBox   := ::cColdBox
      cColor3   := zh_ColorIndex( ::cColorSpec, 0 )
      cColor4   := zh_ColorIndex( ::cColorSpec, 1 )
      cColorAny := zh_ColorIndex( ::cColorSpec, 1 )
   ENDIF
   ...
   IF ::lDropDown
      ...
      zh_DispOutAt( nTop, nLeft, ;
         iif( ::nValue == 0, Space( nSize - 1 ), zh_UPadR( ::aItems[ ::nValue ][ _ITEM_cText ], nSize - 1 ) ), ;
         cColorAny )

      zh_DispOutAt( nTop++, nLeft + nSize - 1, ::cStyle, zh_ColorIndex( ::cColorSpec, 7 ) )
   ...
   IF ::lIsOpen
      IF ! Empty( cHotBox )
         ...
         cColorScrl := zh_ColorIndex( ::cColorSpec, 4 )
         zh_Scroll( nTop, nLeft, ::nBottom, ::nRight,,, cColorScrl )
         zh_DispBox( nTop, nLeft, ::nBottom, ::nRight, cHotBox, cColorScrl )
      ...
      FOR nItem := ::nTopItem TO nEnd
         zh_DispOutAt( nTop++, nLeft, zh_UPadR( ::aItems[ nItem ][ _ITEM_cText ], nSize ), iif( nItem == ::nValue, cColor4, cColor3 ) )
      NEXT
   ENDIF
   ...
   IF ! Empty( cCaption := ::cCaption )
      ...
      zh_DispOutAt( ::nCapRow, ::nCapCol - 1, cCaption, zh_ColorIndex( ::cColorSpec, 5 ) )

      IF nPos != 0
         zh_DispOutAt( ::nCapRow, ::nCapCol + nPos - 2, zh_USubStr( cCaption, nPos, 1 ), zh_ColorIndex( ::cColorSpec, 6 ) )
      ENDIF
   ENDIF
...
METHOD findText( cText, nPos, lCaseSensitive, lExact ) CLASS ListBox
   ...
   zh_default( @nPos, 1 )
   zh_default( @lCaseSensitive, .T. )
   IF ! ZH_ISLOGICAL( lExact )
   ...
   ELSE
      IF lCaseSensitive
         bSearch := {| aItem | zh_LeftEq( aItem[ _ITEM_cText ], cText ) }
      ELSE
         bSearch := {| aItem | zh_LeftEqI( aItem[ _ITEM_cText ], cText ) }
      ENDIF
   ENDIF
...
/* NOTE: Both Cl*pper and Ziher may RTE when searching for
   ...
   ELSE
      IF lCaseSensitive
         bSearch := {| aItem, xItemData | xItemData := _LISTBOX_ITEMDATA( aItem ), ;
            iif( ZH_ISSTRING( xItemData ), zh_LeftEq( xItemData, xData ), ;
                                           xItemData == xData ) }
      ELSE
         /* Cl*pper will also RTE here, if xData is not a string */
         bSearch := {| aItem | zh_LeftEqI( _LISTBOX_ITEMDATA( aItem ), xData ) }
      ENDIF
   ENDIF
...
METHOD insItem( nPos, cText, xData ) CLASS ListBox
   ...
   IF ZH_ISSTRING( cText ) .AND. ;
      ZH_ISNUMERIC( nPos ) .AND. ;
      nPos < ::nItemCount

      zh_AIns( ::aItems, nPos, { cText, xData }, .T. )
      ::nItemCount++
...
METHOD scroll( nMethod ) CLASS ListBox
   ...
   IF ZH_ISNUMERIC( nMethod )
      ...
      CASE HTSCROLLTHUMBDRAG
         ...
         DO WHILE ( ( nKey := zh_keyStd( Inkey( 0 ) ) ) != K_LBUTTONUP )
...
METHOD caption( cCaption ) CLASS ListBox
   ...
   IF cCaption != NIL
      ...
      IF ::nCapCol == NIL
         ::nCapRow := ::nTop
         ::nCapCol := ::nLeft - zh_ULen( ::cCaption )
      ENDIF
   ENDIF
...
METHOD colorSpec( cColorSpec ) CLASS ListBox
   ...
   IF cColorSpec != NIL
      ::cColorSpec := checkVariableTypeAndValidBlock( Self, "COLORSPEC", cColorSpec, "C", 1001, ;
         iif( ::lDropDown, ;
            {|| ! Empty( zh_ColorIndex( cColorSpec, 7 ) ) .AND. Empty( zh_ColorIndex( cColorSpec, 8 ) ) }, ;
...
METHOD style( cStyle ) CLASS ListBox
   ...
   IF cStyle != NIL
      ::cStyle := checkVariableTypeAndValidBlock( Self, "STYLE", cStyle, "C", 1001, {|| zh_ULen( cStyle ) == 1 } )
   ENDIF
...
FUNCTION _ListBox_( nTop, nLeft, nBottom, nRight, xPos, aItems, cCaption, ;
   ...
   IF ( o := HBListBox():New( nTop, nLeft, nBottom, nRight, lDropDown ) ) != NIL
      ...
      IF zh_defaultValue( lScrollBar, .F. )
         IF zh_defaultValue( lDropDown, .F. )
            nTop++
         ENDIF
         o:VScroll := ScrollBar( nTop + 1, nBottom - 1, nRight )
      ENDIF

      IF ZH_ISSTRING( cBitmap )
         o:bitmap := cBitmap
      ENDIF
