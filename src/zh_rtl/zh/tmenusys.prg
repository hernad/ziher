...
/* NOTE: Ziher doesn't support CA-Cl*pper 5.3 GUI functionality, but
...
METHOD Modal( nSelection, nMsgRow, nMsgLeft, nMsgRight, cMsgColor, GetList ) CLASS HBMenuSys
   ...
   IF nSelection <= 0
      ...
      DO WHILE nSelection <= 0
         ...
         nKeyStd := zh_keyStd( nKey := Inkey( 0, zh_bitOr( zh_bitOr( INKEY_KEYBOARD, INKEY_LDOWN ), ZH_INKEY_EXT ) ) )
   ...
   DO WHILE .T.
      ...
      CASE nKeyStd == K_ESC  // go to previous menu
         ...
         IF ::PopMenu()
            ...
            ::ShowMsg( .T. )
         ELSE
            IF IS_IN( ::oMenu:ClassName(), "POPUPMENU|ZH_POPUPMENU" )
               ::oMenu:close()
            ENDIF
...
METHOD PushMenu() CLASS HBMenuSys
   ...
   IF ZH_ISOBJECT( oNewMenu ) .AND. oNewMenu:IsPopUp
...
METHOD Execute() CLASS HBMenuSys
   ...
   IF ZH_ISOBJECT( oNewMenu ) .AND. ! oNewMenu:IsPopUp
      ...
      IF IS_IN( ::oMenu:ClassName(), "TOPBARMENU|POPUPMENU|ZH_POPUPMENU" )
         SetPos( ::nOldRow, ::nOldCol )
         SetCursor( ::nOldCursor )
      ...
      IF IS_IN( ::oMenu:ClassName(), "POPUPMENU|ZH_POPUPMENU" ) .AND. ;
         ::nMenuLevel == 1 .AND. ;
         ! ::oMenu:isOpen
...
METHOD ShowMsg( lMode ) CLASS HBMenuSys
   ...
   IF zh_defaultValue( ::lOldMsgFlag, .F. )
      RestScreen( ::nMsgRow, ::nMsgLeft, ::nMsgRow, ::nMsgRight, ::cMsgSaveS )
   ENDIF
   ...
   IF lMode
      ...
      IF ::lMsgFlag .AND. ;
         ...
         zh_DispOutAt( ::nMsgRow, ::nMsgLeft, zh_UPadC( cMsg, ::nMsgRight - ::nMsgLeft + 1 ), ::cMsgColor )
      ENDIF
