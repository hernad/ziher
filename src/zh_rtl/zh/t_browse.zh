...
/* ZH_BRW_STATICMOUSE controls if mouse position is static
 * and set by call to HitTest() method or dynamic calculated
 * by calls to MPOS() / MCol(). CL53 uses dynamic mouse but
 * I guess that some Ziher GUI libraries inherit from TBrowse
 ...
 */
   ...
   METHOD new( nTop, nLeft, nBottom, nRight )   // constructor, NOTE: This method is a Ziher extension [vszakats]
...
STATIC FUNCTION _SKIP_RESULT( xResult )
   RETURN iif( ZH_ISNUMERIC( xResult ), Int( xResult ), 0 )
   ...
   FOR EACH aCol IN aColData
      IF aCol[ _TBCI_COLPOS ] != NIL
         ...
         IF lFirst
            ...
            cSep := Replicate( zh_URight( cSep, 1 ), nWidth + ;
         ...
         ELSEIF lFirstVisible
            ...
            IF nLen <= aCol[ _TBCI_SEPWIDTH ]
               cSep := Replicate( zh_URight( cSep, 1 ), nWidth + ;
                                  aCol[ _TBCI_FROZENSPACE ] )
            ELSE
               cSep := zh_USubStr( cSep, aCol[ _TBCI_SEPWIDTH ] + 1, nWidth )
               IF ( nLen -= aCol[ _TBCI_SEPWIDTH ] + nWidth ) < 0
                  cSep += Replicate( zh_URight( cSep, 1 ), -nLen )
               ENDIF
               IF aCol[ _TBCI_FROZENSPACE ] > 0
                  cSep := Replicate( zh_ULeft( cSep, 1 ), aCol[ _TBCI_FROZENSPACE ] ) + ;
                                     cSep
               ENDIF
            ENDIF
         ELSE
            nLen := zh_ULen( cSep ) - aCol[ _TBCI_SEPWIDTH ] - nWidth
            IF nLen > 0
               cSep := zh_ULeft( cSep, aCol[ _TBCI_SEPWIDTH ] + nWidth )
            ELSEIF nLen < 0
               cSep += Replicate( zh_URight( cSep, 1 ), -nLen )
            ENDIF
            IF aCol[ _TBCI_FROZENSPACE ] > 0
               cSep := zh_UStuff( cSep, aCol[ _TBCI_SEPWIDTH ] + 1, 0, ;
                                  Replicate( zh_USubStr( cSep, aCol[ _TBCI_SEPWIDTH ] + 1, 1 ), ;
                                             aCol[ _TBCI_FROZENSPACE ] ), cSep )
            ENDIF
         ENDIF
         IF aCol[ _TBCI_LASTSPACE ] > 0
            cSep += Replicate( zh_URight( cSep, 1 ), aCol[ _TBCI_LASTSPACE ] )
         ELSEIF aCol[ _TBCI_LASTSPACE ] < 0
            cSep := zh_ULeft( cSep, zh_ULen( cSep ) + aCol[ _TBCI_LASTSPACE ] )
         ENDIF
         zh_DispOutAtBox( nRow, aCol[ _TBCI_COLPOS ] - aCol[ _TBCI_FROZENSPACE ], ;
                          cSep, cColor )
...
STATIC PROCEDURE _DISP_FHNAME( nRow, nHeight, nLeft, nRight, nType, nColor, aColors, aColData )
   ...
   zh_DispBox( nRow, nLeft, nRow + nHeight - 1, nRight, ;
   ...
   FOR EACH aCol IN aColData
      IF aCol[ _TBCI_COLPOS ] != NIL
         ...
         FOR nPos := 1 TO nHeight
            zh_DispOutAt( nRow + nPos - 1, nCol, ;
                          zh_UPadR( zh_tokenGet( cName, nPos, _TBR_CHR_LINEDELIMITER ), nWidth ), ;
...
METHOD dispFrames() CLASS TBrowse
   ...
   IF ::lInvalid .AND. ! Empty( ::cBorder )
      zh_DispBox( ::nTop, ::nLeft, ::nBottom, ::nRight, ::cBorder, ::colorValue( _TBC_CLR_STANDARD ) )
   ENDIF
...
METHOD dispRow( nRow ) CLASS TBrowse
   ...
   IF nRow >= 1 .AND. nRow <= ::rowCount
      ...
      zh_DispBox( nRowPos, ::n_Left, nRowPos, ::n_Right, Space( 9 ), cStdColor )
      ...
      FOR EACH aCol, cValue, aColors IN ::aColData, ::aCellValues[ nRow ], ::aCellColors[ nRow ]
         IF aCol[ _TBCI_COLPOS ] != NIL
            ...
            ELSEIF aCol[ _TBCI_SEPWIDTH ] > 0
               zh_DispOutAtBox( nRowPos, aCol[ _TBCI_COLPOS ] - aCol[ _TBCI_FROZENSPACE ], ;
            ...
            IF aCol[ _TBCI_LASTSPACE ] < 0
               zh_DispOutAt( nRowPos, nColPos, ;
                             zh_ULeft( cValue, ::n_Right - nColPos + 1 ), cColor )
            ELSE
               zh_DispOutAt( nRowPos, nColPos, cValue, cColor )
            ENDIF
         ENDIF
      NEXT
...
METHOD colorRect( aRect, aColors ) CLASS TBrowse
   ...
   IF ZH_ISARRAY( aRect ) .AND. Len( aRect ) >= 4 .AND. ;
      ZH_ISNUMERIC( aRect[ 1 ] ) .AND. ZH_ISNUMERIC( aRect[ 2 ] ) .AND. ;
      ZH_ISNUMERIC( aRect[ 3 ] ) .AND. ZH_ISNUMERIC( aRect[ 4 ] ) .AND. ;
      ...
      ;
      ZH_ISARRAY( aColors ) .AND. Len( aColors ) >= 2 .AND. ;
      ZH_ISNUMERIC( aColors[ 1 ] ) .AND. ZH_ISNUMERIC( aColors[ 2 ] ) .AND. ;
...
METHOD scrollBuffer( nRows ) CLASS TBrowse
   ...
   IF nRows >= nRowCount .OR. nRows <= -nRowCount
      AFill( ::aCellStatus, .F. )
   ELSE
      zh_Scroll( ::n_Top + ::nHeadHeight + iif( ::lHeadSep, 1, 0 ), ::n_Left, ;
      ...
      ELSEIF nRows < 0
         DO WHILE ++nRows <= 0
            zh_AIns( ::aCellValues, 1, ATail( ::aCellValues ), .F. )
            zh_AIns( ::aCellColors, 1, ATail( ::aCellColors ), .F. )
            zh_AIns( ::aCellStatus, 1, .F., .F. )
            zh_AIns( ::aDispStatus, 1, .T., .F. )
         ENDDO
      ENDIF
   ENDIF
      ...
      IF nRow <= ::nLastRow
         FOR EACH aCol, cValue, aColor IN ::aColData, ::aCellValues[ nRow ], ::aCellColors[ nRow ]
            ...
            IF ValType( cValue ) $ "CMNDTL"
               cValue := zh_UPadR( Transform( cValue, iif( ZH_ISSTRING( oCol:picture ), oCol:picture, NIL ) ), aCol[ _TBCI_CELLWIDTH ] )
            ELSE
...
METHOD forceStable() CLASS TBrowse
    ...
    *       to revert my stupid modifications in Ziher core code. Looking
...
STATIC FUNCTION _DECODECOLORS( cColorSpec )
   LOCAL aColors := {}
   LOCAL nColors := zh_tokenCount( cColorSpec, "," )
   LOCAL cColor
   LOCAL nPos

   FOR nPos := 1 TO nColors
      cColor := zh_tokenGet( cColorSpec, nPos, "," )
       ...
       * In Ziher this validation is redirected to GT system which
       ...
       */
      IF nPos <= 2 .AND. zh_ColorToN( cColor ) == -1
...
STATIC FUNCTION _COLDEFCOLORS( aDefColorsIdx, nMaxColorIndex )
   ...
   IF ZH_ISARRAY( aDefColorsIdx )
      FOR nPos := 1 TO _TBC_CLR_MAX
         IF nPos <= Len( aDefColorsIdx ) .AND. ;
            ZH_ISNUMERIC( nColorIndex := aDefColorsIdx[ nPos ] ) .AND. ;
...
METHOD configure( nMode ) CLASS TBrowse
   ...
   IF ! ZH_ISNUMERIC( nMode ) .OR. nMode == 0 .OR. nMode > _TBR_CONF_ALL
      nMode := _TBR_CONF_ALL
   ENDIF
   ::nConfigure := zh_bitOr( ::nConfigure, nMode )
...
METHOD doConfigure() CLASS TBrowse
   ...
   FOR EACH oCol, aCol IN ::columns, ::aColData
      ...
      nWidth := iif( cType $ "CMNDTL", ;
                     zh_ULen( Transform( xValue, iif( ZH_ISSTRING( oCol:picture ), oCol:picture, NIL ) ) ), 0 )
      cColSep := oCol:colSep
      IF cColSep == NIL
         cColSep := ::cColSep
      ENDIF
      cHeadSep := zh_defaultValue( oCol:headSep, "" )
      IF cHeadSep == ""
         cHeadSep := zh_defaultValue( ::cHeadSep, "" )
      ENDIF
      cFootSep := zh_defaultValue( oCol:footSep, "" )
      IF cFootSep == ""
         cFootSep := zh_defaultValue( ::cFootSep, "" )
      ENDIF
      ...
      aCol[ _TBCI_SEPWIDTH    ] := zh_ULen( cColSep )
   ...
   FOR EACH aCol IN ::aColData
      aCol[ _TBCI_HEADING ] := Replicate( _TBR_CHR_LINEDELIMITER, nHeadHeight - zh_tokenCount( aCol[ _TBCI_HEADING ], _TBR_CHR_LINEDELIMITER ) ) + ;
                               aCol[ _TBCI_HEADING ]
...
STATIC PROCEDURE _GENLIMITRTE()
   ...
   oError:subCode     := 0
   oError:description := zh_langErrMsg( EG_LIMIT )
   oError:canRetry    := .F.
   oError:canDefault  := .F.
   oError:fileName    := ""
   oError:osCode      := 0
...
STATIC FUNCTION _DECODE_FH( cName, nHeight, nWidth )
   ...
   IF ZH_ISSTRING( cName )
      ...
      IF ! cName == ""
          ...
          */
         IF zh_URight( cName, 1 ) == _TBR_CHR_LINEDELIMITER
            cName := zh_StrShrink( cName )  /* FIXME: use zh_UStrShrink() */
         ENDIF
         nHeight := zh_tokenCount( cName, _TBR_CHR_LINEDELIMITER )
         FOR i := 1 TO nHeight
            nWidth := Max( nWidth, zh_ULen( zh_tokenGet( cName, i, _TBR_CHR_LINEDELIMITER ) ) )
         NEXT
      ENDIF
...
METHOD hiLite() CLASS TBrowse
   ...
   IF ::setCursorPos()
      IF ( cValue := ::cellValue( ::nRowPos, ::nColPos ) ) != NIL
         ...
         IF ::n_Col + zh_ULen( cValue ) > _TBR_COORD( ::n_Right )
            cValue := zh_ULeft( cValue, _TBR_COORD( ::n_Right ) - ::n_Col + 1 )
         ENDIF
         zh_DispOutAt( ::n_Row, ::n_Col, cValue, cColor )
         SetPos( ::n_Row, ::n_Col )
         ::lHiLited := .T.
      ENDIF
   ENDIF
...
METHOD colWidth( nColumn ) CLASS TBrowse
   ...
   IF ZH_ISNUMERIC( nColumn ) .AND. nColumn >= 1 .AND. nColumn <= ::colCount
...
METHOD insColumn( nColumn, oCol ) CLASS TBrowse
   ...
   IF nColumn >= 1 .AND. nColumn <= ::colCount + 1
      zh_AIns( ::columns, nColumn, oCol, .T. )
      ::configure( _TBR_CONF_COLUMNS )
   ENDIF
...
METHOD hitTest( mRow, mCol ) CLASS TBrowse
   ...
   IF ! ZH_ISNUMERIC( mRow ) .OR. ! ZH_ISNUMERIC( mCol ) .OR. ;
...
METHOD border( cBorder ) CLASS TBrowse
   ...
   IF cBorder != NIL
      ...
      IF cBorder == "" .OR. zh_ULen( cBorder ) == 8
...
METHOD setStyle( nStyle, lNewValue ) CLASS TBrowse
   ...
   zh_default( @::styles, { .F., .F., .F., .F., .F., NIL } )
