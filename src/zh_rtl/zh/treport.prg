...
METHOD PROCEDURE New( cFrmName AS STRING, ;
   ...
   ELSE
      /* NOTE: CA-Cl*pper does an RTrim() on the filename here,
               but in Ziher we're using _SET_TRIMFILENAME. */
      IF Set( _SET_DEFEXTENSIONS )
         cFRMName := zh_FNameExtSetDef( cFRMName, ".frm" )
      ENDIF
   ENDIF
   ...
   IF ZH_ISSTRING( cAltFile ) .AND. ! cAltFile == ""   // To file
   ...
   IF ZH_ISSTRING( cAltFile ) .AND. ! cAltFile == ""       // Set extrafile back
         ...
         FOR EACH cLine IN aTempPgHeader
            ...
            FOR nHeadLine := 1 TO nLinesInHeader
               AAdd( aPageHeader, Space( 15 ) + ;
                  zh_UPadC( RTrim( XMEMOLINE( LTrim( cLine ), ;
                  nHeadingLength, nHeadLine ) ), nHeadingLength ) )
            NEXT
         NEXT

         aPageHeader[ 1 ] := zh_UStuff( aPageHeader[ 1 ], 1, 14, ;
   ...
   FOR EACH aCol IN ::aReportData[ RPT_COLUMNS ]    // Cycle through the columns
      FOR nLine := 1 TO nMaxColLength
         ...
         ELSEIF aCol[ RCT_TYPE ] == "N"
            aPageHeader[ nLinesInHeader + nLine ] += ;
               zh_UPadL( aCol[ RCT_HEADER ][ nLine ], aCol[ RCT_WIDTH ] )
         ELSE
            aPageHeader[ nLinesInHeader + nLine ] += ;
               zh_UPadR( aCol[ RCT_HEADER ][ nLine ], aCol[ RCT_WIDTH ] )
         ENDIF
      NEXT
   NEXT
   ...
   IF ! ::lFirstPass                       // Don't bother first time through
      ...
      FOR nGroup := Len( ::aReportData[ RPT_GROUPS ] ) TO 1 STEP -1
         ...
         IF lGroupChanged .OR. MakeAStr( Eval( ::aReportData[ RPT_GROUPS ][ nGroup ][ RGT_EXP ] ), ;
            ...
            aRecordHeader[ Len( aRecordHeader ) ] := zh_StrShrink( ATail( aRecordHeader ) )  /* FIXME: use zh_UStrShrink() */
         ENDIF
      NEXT
   ENDIF
   ...
   IF ! ::aReportData[ RPT_SUMMARY ]
      ...
      FOR EACH aCol IN ::aReportData[ RPT_COLUMNS ]
         ...
         CASE aCol[ RCT_TYPE ] $ "C"
            nMaxLines := Max( XMLCOUNT( StrTran( Eval( aCol[ RCT_EXP ] ), ";", zh_eol() ), aCol[ RCT_WIDTH ] ), nMaxLines )
         ENDCASE
      NEXT
            ...
            IF aCol[ RCT_TYPE ] $ "CM"
               ...
               ELSE
                  cLine := XMEMOLINE( RTrim( Eval( aCol[ RCT_EXP ] ) ), ;
                     aCol[ RCT_WIDTH ], nLine )
               ENDIF
               cLine := zh_UPadR( cLine, aCol[ RCT_WIDTH ] )
            ELSE
               IF nLine == 1
...
METHOD LoadReportFile( cFrmFile AS STRING ) CLASS HBReportForm
   ...
   IF ( hFile := zh_vfOpen( cFrmFile, ZH_FO_DEFAULTS ) ) == NIL
      err := ErrorNew()
      err:severity := ES_ERROR
      err:genCode := EG_OPEN
      err:subSystem := "FRMLBL"
      ...
      Eval( ErrorBlock(), err )
   ELSE
      IF zh_vfRead( hFile, @cFileBuff, SIZE_FILE_BUFF ) > 0 .AND. FError() == 0
         // Is this a .frm type file (2 at start and end of file)
         IF Bin2W( zh_BSubStr( cFileBuff, 1, 2 ) ) == 2 .AND. ;
            Bin2W( zh_BSubStr( cFileBuff, SIZE_FILE_BUFF - 1, 2 ) ) == 2

            // Fill processing buffers
            ::cLengthsBuff := zh_BSubStr( cFileBuff, LENGTHS_OFFSET, SIZE_LENGTHS_BUFF )
            ::cOffSetsBuff := zh_BSubStr( cFileBuff, OFFSETS_OFFSET, SIZE_OFFSETS_BUFF )
            ::cExprBuff    := zh_BSubStr( cFileBuff, EXPR_OFFSET, SIZE_EXPR_BUFF )
            cFieldsBuff    := zh_BSubStr( cFileBuff, FIELDS_OFFSET, SIZE_FIELDS_BUFF )
            cParamsBuff    := zh_BSubStr( cFileBuff, PARAMS_OFFSET, SIZE_PARAMS_BUFF )

            // Process report attributes
            // Report width
            aReport[ RPT_WIDTH ]   := Bin2W( zh_BSubStr( cParamsBuff, PAGE_WIDTH_OFFSET, 2 ) )

            // Lines per page
            aReport[ RPT_LINES ]   := Bin2W( zh_BSubStr( cParamsBuff, LNS_PER_PAGE_OFFSET, 2 ) )

            // Page offset (left margin)
            aReport[ RPT_LMARGIN ] := Bin2W( zh_BSubStr( cParamsBuff, LEFT_MRGN_OFFSET, 2 ) )

            // Page right margin (not used)
            aReport[ RPT_RMARGIN ] := Bin2W( zh_BSubStr( cParamsBuff, RIGHT_MGRN_OFFSET, 2 ) )

            nColCount := Bin2W( zh_BSubStr( cParamsBuff, COL_COUNT_OFFSET, 2 ) )

            // Line spacing
            // Spacing is 1, 2, or 3
            aReport[ RPT_SPACING ] := iif( zh_BSubStr( cParamsBuff, ;
               DBL_SPACE_OFFSET, 1 ) $ "YyTt", 2, 1 )

            // Summary report flag
            aReport[ RPT_SUMMARY ] := zh_BSubStr( cParamsBuff, SUMMARY_RPT_OFFSET, 1 ) $ "YyTt"

            // Process report eject and plain attributes option byte
            nOptionByte := zh_BPeek( cParamsBuff, OPTION_OFFSET )

               IF zh_bitAnd( nOptionByte, 4 ) != 0
                  aReport[ RPT_PLAIN ] := .T.          // Plain page
               ENDIF
               IF zh_bitAnd( nOptionByte, 2 ) != 0
                  aReport[ RPT_AEJECT ] := .T.         // Page eject after report
               ENDIF
               IF zh_bitAnd( nOptionByte, 1 ) != 0
            ...
            nHeaderIndex := 4
            aHeader := ParseHeader( ::GetExpr( Bin2W( zh_BSubStr( cParamsBuff, PAGE_HDR_OFFSET, 2 ) ) ), nHeaderIndex )
            ...
            // Process Groups
            // Group
            IF ! Empty( cGroupExp := ::GetExpr( Bin2W( zh_BSubStr( cParamsBuff, GRP_EXPR_OFFSET, 2 ) ) ) )
               ...
               aReport[ RPT_GROUPS ][ 1 ][ RGT_EXP ] := zh_macroBlock( cGroupExp )
               IF Used()
               ...
               aReport[ RPT_GROUPS ][ 1 ][ RGT_HEADER ] := ;
                  ::GetExpr( Bin2W( zh_BSubStr( cParamsBuff, GRP_HDR_OFFSET, 2 ) ) )

               // Page eject after group
               aReport[ RPT_GROUPS ][ 1 ][ RGT_AEJECT ] := zh_BSubStr( cParamsBuff, ;
                  PE_OFFSET, 1 ) $ "YyTt"
            ENDIF

            // Subgroup
            IF ! Empty( cSubGroupExp := ::GetExpr( Bin2W( zh_BSubStr( cParamsBuff, SUB_EXPR_OFFSET, 2 ) ) ) )
               ...
               aReport[ RPT_GROUPS ][ 2 ][ RGT_EXP ] := zh_macroBlock( cSubGroupExp )
               IF Used()
               ...
               aReport[ RPT_GROUPS ][ 2 ][ RGT_HEADER ] := ;
                  ::GetExpr( Bin2W( zh_BSubStr( cParamsBuff, SUB_HDR_OFFSET, 2 ) ) )
...
/* Reads an expression from EXPR_BUFF via the OFFSETS_BUFF and returns
   ...
   Notes:
      1. The expression is empty if:
          ...
          b. Character following character pointed to by pointer is zh_BChar( 0 ) */
METHOD GetExpr( nPointer AS NUMERIC ) CLASS HBReportForm
   ...
   IF nPointer != 65535
      ...
      nExprOffset := Bin2W( zh_BSubStr( ::cOffsetsBuff, nOffsetOffset, 2 ) )
      nExprLength := Bin2W( zh_BSubStr( ::cLengthsBuff, nOffsetOffset, 2 ) )
...
STATIC FUNCTION Occurs( cSearch, cTarget )
   ...
   DO WHILE ( nPos := zh_UAt( cSearch, cTarget ) ) > 0
      nCount++
      cTarget := zh_USubStr( cTarget, nPos + 1 )
   ENDDO
...
STATIC FUNCTION XMLCOUNT( cString, nLineLength, nTabSize, lWrap )
   ...
   zh_default( @nLineLength, 79 )
   zh_default( @nTabSize, 4 )
...
STATIC FUNCTION ParseHeader( cHeaderString, nFields )
   ...
   DO WHILE ++nItemCount <= nFields
      ...
      cItem := zh_ULeft( cHeaderString, nHeaderLen )

      // check for explicit delimiter
      IF ( nPos := zh_UAt( ";", cItem ) ) > 0
         // delimiter present
         AAdd( aPageHeader, zh_ULeft( cItem, nPos - 1 ) )
      ELSE
      ...
      cHeaderString := zh_USubStr( cHeaderString, nPos + 1 )
...
METHOD GetColumn( cFieldsBuffer AS STRING, /* @ */ nOffset AS NUMERIC ) CLASS HBReportForm
   ...
   // Column width
   aColumn[ RCT_WIDTH ] := Bin2W( zh_BSubStr( cFieldsBuffer, nOffset + ;
      FIELD_WIDTH_OFFSET, 2 ) )

   // Total column?
   aColumn[ RCT_TOTAL ] := ;
      zh_BSubStr( cFieldsBuffer, nOffset + FIELD_TOTALS_OFFSET, 1 ) $ "YyTt"

   // Decimals width
   aColumn[ RCT_DECIMALS ] := Bin2W( zh_BSubStr( cFieldsBuffer, nOffset + ;
   ...
   aColumn[ RCT_TEXT ] := ::GetExpr( Bin2W( ;
      zh_BSubStr( cFieldsBuffer, nOffset + FIELD_CONTENT_EXPR_OFFSET, 2 ) ) )
   aColumn[ RCT_EXP ] := zh_macroBlock( aColumn[ RCT_TEXT ] )

   // Header expression
   aColumn[ RCT_HEADER ] := zh_ATokens( ::GetExpr( Bin2W( ;
      zh_BSubStr( cFieldsBuffer, nOffset + FIELD_HEADER_EXPR_OFFSET, 2 ) ) ), ";" )
...
STATIC FUNCTION MakeAStr( uVar, cType )
   ...
   CASE Asc( "T" )
   CASE Asc( "t" ) ; RETURN zh_TToC( uVar )
   CASE Asc( "L" )
