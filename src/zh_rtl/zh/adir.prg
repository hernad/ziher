...
FUNCTION ADir( cFileMask, aName, aSize, aDate, aTime, aAttr )
   ...
   IF ! ZH_ISSTRING( cFileMask )
      RETURN 0
   ENDIF

   // If no drive/dir specified, use the SET DEFAULT setting.

   zh_FNameSplit( cFileMask, @cDir, @cName, @cExt )

   IF cDir == ""
      cFileMask := zh_FNameMerge( __DefPath(), cName, cExt )
   ENDIF
   ...
   aDir := zh_vfDirectory( cFileMask, iif( ZH_ISARRAY( aAttr ), "HSD", ) )
   nDirLen := Len( aDir )
   ...
   FOR nDirPos := 1 TO nDirLen
      ...
      IF nDateLen != NIL .AND. nDateLen >= nDirPos
         aDate[ nDirPos ] := zh_TToD( aFileInfo[ F_DATE ] )
      ENDIF
