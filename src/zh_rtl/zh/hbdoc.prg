      ...
      IF zh_vfDirExists( cDir + _HBDOC_SRC_SUBDIR )
         ...
         FOR EACH aFile IN ASort( zh_vfDirectory( cDir + _HBDOC_SRC_SUBDIR + zh_ps() + zh_osFileMask(), "D" ),,, {| tmp1, tmp2 | tmp1[ F_NAME ] < tmp2[ F_NAME ] } )
            IF "D" $ aFile[ F_ATTR ] .AND. ;
               ...
               __hbdoc__read_langdir( aEntry, cDir + _HBDOC_SRC_SUBDIR + zh_ps() + aFile[ F_NAME ], hMeta, aErrMsg )
            ENDIF
         NEXT
      ENDIF
   ENDIF
...
STATIC PROCEDURE __hbdoc__read_langdir( aEntry, cDir, hMeta, aErrMsg )
   ...
   FOR EACH aFile IN ASort( zh_vfDirectory( cDir + zh_ps() + "*" + _HBDOC_SRC_EXT ),,, {| tmp1, tmp2 | tmp1[ F_NAME ] < tmp2[ F_NAME ] } )
      __hbdoc__read_file( aEntry, cDir + zh_ps() + aFile[ F_NAME ], hMeta, aErrMsg )
      ++nCount
   NEXT
...
STATIC PROCEDURE __hbdoc__read_file( aEntry, cFileName, hMeta, aErrMsg )
   ...
   __hbdoc__read_stream( aEntry, zh_UTF8ToStr( MemoRead( cFileName ) ), cFileName, hMeta, aErrMsg )
