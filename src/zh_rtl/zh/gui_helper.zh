...
FUNCTION _GetNumCol( cColor )
   ...
   IF ( nPos := zh_BAt( "/", cColor ) ) > 0
      cColor := zh_BLeft( cColor, nPos - 1 )
   ENDIF
   IF ( nPos := zh_BAt( ",", cColor ) ) > 0
      cColor := zh_BLeft( cColor, nPos - 1 )
   ENDIF

   RETURN zh_AScan( { "B", "G", "BG", "R", "RB", "GR", "W", "N+", "B+", "G+", "BG+", "R+", "RB+", "GR+", "W+" }, cColor,,, .T. )

FUNCTION __GUIColor( cColor, nPos )
   RETURN zh_ColorIndex( cColor, nPos - 1 )
...
FUNCTION __Caption( cCaption )
   ...
   RETURN iif( ( nPos := zh_UAt( "&", cCaption ) ) > 0, zh_UStuff( cCaption, nPos, 1, "" ), cCaption )

FUNCTION __CapLength( cCaption )

   LOCAL nCaptionLen := zh_ULen( cCaption )
   LOCAL nPos

   RETURN iif( ( nPos := zh_UAt( "&", cCaption ) ) > 0 .AND. nPos < nCaptionLen, nCaptionLen - 1, nCaptionLen )
