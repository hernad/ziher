...
STATIC FUNCTION __CPWinToCPStd( nCPWin )
   ...
   CASE 1256
   CASE 1257
   CASE 1258  ; RETURN "cp" + zh_ntos( nCPWin )
   CASE 28591
   CASE 28592
   CASE 28593
   CASE 28594
   ...
   CASE 28599
   CASE 28603
   CASE 28605 ; RETURN "iso8859-" + zh_ntos( nCPWin - 28590 )
...
STATIC FUNCTION __UnixParseLangCP( cString, /* @ */ cLang )
   ...
   cCP := zh_StrReplace( Lower( cCP ), { "_" => "", "-" => "", "ibm" => "cp", "windows" => "cp" } )
   IF zh_LeftEq( cCP, "iso8859" )
      cCP := Stuff( cCP, Len( "iso8859" ) + 1, 0, "-" )
   ENDIF

   /* Convert Unix CP name to Ziher CP ID */
   SWITCH cCP
   CASE "utf8"
   CASE "cp437"
   CASE "cp737"
...
STATIC FUNCTION __CPStdToHb( cCPStd, cCtryStd )
   ...
   IF cCPStd != NIL
      ...
      OTHERWISE
         ...
         FOR EACH cdp IN aCP
            IF zh_LeftEq( cdp, cCtryHb ) .AND. cCPStd == zh_cdpUniID( cdp )
               RETURN cdp
            ENDIF
         NEXT
         FOR EACH cdp IN aCP
            IF cCPStd == zh_cdpUniID( cdp )
               RETURN zh_cdpUniID( cdp )
            ENDIF
         NEXT
      ENDSWITCH
   ENDIF
...
STATIC FUNCTION __LangStdToCPCtryHb( cCtryStd )
   ...
   RETURN Left( zh_cdpSelect(), 2 )  /* Caller assumes this never returns strings shorter than two chars */
