 ...
 * Copyright 2009 Przemyslaw Czerpak (zh_timeFormat(), zh_timeUnformat(), zh_timeStampFormat(), zh_timeStampUnformat())
 *
...
char * zh_dateFormat( const char * szDate, char * szFormattedDate, const char * szDateFormat )
{
   /*
   ...
   ZH_TRACE( ZH_TR_DEBUG, ( "zh_dateFormat(%s, %p, %s)", szDate, ( void * ) szFormattedDate, szDateFormat ) );
   ...
   {
      const char * szPtr;
      ZH_BOOL used_d, used_m, used_y;

      format_count = 0;
      used_d = used_m = used_y = ZH_FALSE;
      szPtr = szDateFormat;

      while( format_count < size )
      {
         int digit = ZH_TOUPPER( ( ZH_UCHAR ) *szPtr );
         szPtr++;
         digit_count = 1;
         while( ZH_TOUPPER( ( ZH_UCHAR ) *szPtr ) == digit && format_count < size )
         {
            szPtr++;
   ...
   else
   {
      ...
      zh_strncpy( szFormattedDate, szDateFormat, size );
...
static int zh_dateUnformatRaw( const char * szDate, const char * szDateFormat, long * plDate )
{
   int d_value = 0, m_value = 0, y_value = 0;
   int iSize = 0;

   ZH_TRACE( ZH_TR_DEBUG, ( "zh_dateUnformatRaw(%s, %s, %p)", szDate, szDateFormat, ( void * ) plDate ) );
   ...
   if( szDate )
   {
      ...
      y_value = zh_setUpdateEpoch( y_value );
   }

   *plDate = zh_dateEncode( y_value, m_value, d_value );

   return iSize;
}

long zh_dateUnformat( const char * szDate, const char * szDateFormat )
{
   long lDate;

   ZH_TRACE( ZH_TR_DEBUG, ( "zh_dateFormat(%s, %s)", szDate, szDateFormat ) );

   zh_dateUnformatRaw( szDate, szDateFormat, &lDate );
 ...
 */
char * zh_timeFormat( char * szBuffer, const char * szTimeFormat, long lMilliSec )
{
   char * szTimeBuffer;
   int iHour, iMinutes, iSeconds, iMSec, iPM, i12;
   int size, i, value, digits, skip;

   ZH_TRACE( ZH_TR_DEBUG, ( "zh_timeFormat(%p, %s, %ld)", ( void * ) szBuffer, szTimeFormat, lMilliSec ) );

   zh_timeDecode( lMilliSec, &iHour, &iMinutes, &iSeconds, &iMSec );
   szTimeBuffer = szBuffer;

   size = ( int ) zh_strnlen( szTimeFormat, 16 );
   iPM = i12 = 0;
   for( i = 0; i < size; ++i )
   {
      if( ZH_TOUPPER( szTimeFormat[ i ] ) == 'P' )
      {
         if( iHour >= 12 )
         {
            iPM = 1;
   ...
   {
      int count = -i;
      int ch = ZH_TOUPPER( szTimeFormat[ i ] );
      ++i;
      while( ch == ZH_TOUPPER( szTimeFormat[ i ] ) && i < size )
         ++i;
      count += i;
      switch( ch )
      {
 ...
 */
char * zh_timeStampFormat( char * szBuffer,
                           const char * szDateFormat, const char * szTimeFormat,
                           long lJulian, long lMilliSec )
{
   char szDate[ 9 ], * szTimeBuffer;

   ZH_TRACE( ZH_TR_DEBUG, ( "zh_timeStampFormat(%p, %s, %s, %ld, %ld)", ( void * ) szBuffer, szDateFormat, szTimeFormat, lJulian, lMilliSec ) );

   zh_dateDecStr( szDate, lJulian );
   zh_dateFormat( szDate, szBuffer, szDateFormat );
   szTimeBuffer = szBuffer + strlen( szBuffer );
   if( *szBuffer )
      *szTimeBuffer++ = ' ';
   zh_timeFormat( szTimeBuffer, szTimeFormat, lMilliSec );

   return szBuffer;
}

long zh_timeUnformat( const char * szTime, const char * szTimeFormat )
{
   int iHour, iMinutes, iSeconds, iMSec, iPM;
   int size, i, count, prec, * pValue;

   ZH_TRACE( ZH_TR_DEBUG, ( "zh_timeUnformat(%s, %s)", szTime, szTimeFormat ) );
   ...
   size = ( int ) zh_strnlen( szTime, zh_strnlen( szTimeFormat, 16 ) );
   ...
   for( i = count = 0; i < size && szTime[ count ]; ++i )
   {
      switch( szTimeFormat[ i ] )
      {
         ...
         case 'p':
            if( iPM < 0 )
            {
               while( szTime[ count ] && ! ZH_ISDIGIT( szTime[ count ] ) &&
   ...
   return zh_timeEncode( iHour, iMinutes, iSeconds, iMSec );
}

void zh_timeStampUnformat( const char * szDateTime,
                           const char * szDateFormat, const char * szTimeFormat,
                           long * plJulian, long * plMilliSec )
{
   ZH_TRACE( ZH_TR_DEBUG, ( "zh_timeStampUnformat(%s, %s, %s, %p, %p)", szDateTime, szDateFormat, szTimeFormat, ( void * ) plJulian, ( void * ) plMilliSec ) );
   ...
   if( szDateTime )
   {
      ...
      if( ! szDateFormat )
         szDateFormat = zh_setGetDateFormat();
      size = zh_dateUnformatRaw( szDateTime, szDateFormat, plJulian );
      *plMilliSec = zh_timeUnformat( szDateTime + size, szTimeFormat );
   }
   else
