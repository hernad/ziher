 ...
 * Ziher adaptation Copyright 2011 Tamas TEVESZ <ice@extreme.hu>
 */
...
static ZH_I32 arc4_count;

static ZH_CRITICAL_NEW( arc4_lock );
#define ARC4_LOCK()    zh_threadEnterCriticalSection( &arc4_lock )
#define ARC4_UNLOCK()  zh_threadLeaveCriticalSection( &arc4_lock )

#if defined( __BORLANDC__ ) && defined( _ZH_INLINE_ )
#undef _ZH_INLINE_
#define _ZH_INLINE_
#endif

static _ZH_INLINE_ ZH_U8 arc4_getbyte( void );

static _ZH_INLINE_ void arc4_init( void )
{
   int n;
...
static _ZH_INLINE_ void arc4_addrandom( const ZH_U8 * dat, int datlen )
{
   int n;
...
static _ZH_INLINE_ int hex_char_to_int( char c )
{
   switch( c )
   {
      case '0':           return 0;
...
static int arc4_seed_rand( void )
{
   ...
   srand( ( unsigned ) zh_dateMilliSeconds() );

   for( i = 0; i < sizeof( buf ); i++ )
      buf[ i ] = ( ZH_U8 ) ( rand() % 256 );  /* not biased */
...
static _ZH_INLINE_ ZH_U32 arc4_getword( void )
{
   ZH_U32 val;
...
#if 0
/*
 * These two are part of the original arc4random API, but Ziher does not
...
void zh_arc4random_buf( void * _buf, ZH_SIZE n )
{
   ZH_U8 * buf = ( ZH_U8 * ) _buf;
 ...
 */
ZH_U32 zh_arc4random_uniform( ZH_U32 upper_bound )
{
   ZH_U32 r, min;
