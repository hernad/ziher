...
static signed char base64_decode_value( int value_in )
{
   ...
   value_in -= 43;
   if( value_in < 0 || value_in >= ( int ) ZH_SIZEOFARRAY( s_decoding ) )
      return -1;

   return s_decoding[ value_in ];
}

static ZH_SIZE base64_decode_block( const char * code_in, const ZH_SIZE length_in, char * pszPlainttextOut )
{
...
ZH_FUNC( ZH_BASE64DECODE )
{
   ZH_SIZE nSrcLen = zh_parclen( 1 );

   if( nSrcLen > 0 )
   {
      ZH_SIZE nDstLen = ( ( ( nSrcLen * 3 ) / 4 ) + 1 ) * sizeof( char );
      char *  code    = ( char * ) zh_xgrab( nDstLen );

      nDstLen = base64_decode_block( zh_parcx( 1 ), nSrcLen, code );
      zh_retclen_buffer( code, nDstLen );
   }
   else
      zh_retc_null();
}
