...
ZH_FUNC( SETPOS )  /* Sets the screen position */
{
   if( ZH_IS_PARAM_NUM( 1 ) && ZH_IS_PARAM_NUM( 2 ) )
      zh_gtSetPos( zh_parni( 1 ), zh_parni( 2 ) );

   zh_itemReturn( zh_param( 1, ZH_IT_ANY ) );
}

ZH_FUNC( ROW )  /* Return the current screen row position (zero origin) */
{
   int iRow;
   int iCol;
...
ZH_FUNC( COL )  /* Return the current screen column position (zero origin) */
{
   int iRow;
   int iCol;
