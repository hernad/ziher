...
parseDocEn(path.join(__dirname, "zh_doc"));
parseZHH(path.join(__dirname, "ziher_funcs.zhh"));
...
function createDoc() {
	...
	fs.writeFile("zh_docs.missing", msg, (err) => { if (err) console.error(err); });
	fs.writeFile("zh_docs.json", JSON.stringify(docs, undefined, 1), (err) => { if (err) console.error(err); });
}
